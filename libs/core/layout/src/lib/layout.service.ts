...
export class LayoutService {
  ...
  constructor(private readonly breakpointObserver: BreakpointObserver) {
    this.breakpointObserver
      ...
      .pipe(
        tap((result) => {
          const type = Object.keys(result.breakpoints).find((query) => result.breakpoints[query]);
          const layout = type ? LAYOUT_SHORT_TYPES_MAP[type] ?? Breakpoints.Handset : Breakpoints.Handset;
